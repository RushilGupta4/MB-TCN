...
class MultiBranchingLayer(nn.Module):
    def __init__(self, n_inputs, n_outputs, n_branches):
        ...
        self.branches = nn.ModuleList(
            [nn.Linear(n_inputs, n_outputs) for _ in range(n_branches)]
        )
...
class MBTCN(nn.Module):
    def __init__(
        self, num_inputs, num_channels, n_outputs, n_branches, kernel_size, dropout
    ):
        ...
        self.mask_tcn = TemporalConvNet(
            num_inputs, [8], kernel_size=kernel_size, dropout=dropout
        )
        self.multi_branch = MultiBranchingLayer(
            num_channels[-1] + 8, n_outputs, n_branches
        )
