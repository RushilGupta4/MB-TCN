...
def train_model(
    sublist, val_loader, model, criterion, optimizer, num_epochs, MB_NUM, batch_size
):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(MB_NUM):
            train_set = TensorDataset(
                sublist[i][0].clone().detach().to(device),
            ...
            for inputs, labels, masks in train_loader:
                ...
                final_outputs = torch.reshape(
                    torch.mean(torch.stack(outputs), dim=0), (-1,)
                )
...
def evaluate_model(data_loader, model, criterion):
    ...
    print(
        f"Loss: {avg_loss:.4f} - Accuracy: {accuracy:.4f} - Recall: {recall:.4f} - Precision: {precision:.4f} "
